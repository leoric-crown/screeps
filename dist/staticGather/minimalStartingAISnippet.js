...
​
module.exports.loop = () => {
    // Clear creeps from memory every 100 ticks
	if ((Game.time % 100) === 0) {
...
​
    // add creepts to a (hash) 2d array/dictionary based on the room they are on
	var creeps = {};
...
​
    // execute some logic on every single room i have a creep or owned structure on
...
​
        // my towers attack hostile creeps or heal wounded creeps
...
​
        // creep behaviour
        // first see if the creep has gathered as much energy as it can carry
		for (let creep of creeps[roomIndex]) {
            if (creep.memory.full) {
                // replace this with creep.store.getFreeCapacity(RESOURCE_ENERGY) === 0, creep.carry is deprecated
                if (creep.carry.energy === 0) {
                    creep.memory.full = false;
                }
            } else {
                // replace this with creep.store[RESOURCE_ENERGY] === creep.store.getCapacity()
...
​
            if (creep.memory.full) {
                do {
                    // have the creep drop the energy at a spawn/extension/tower
					if (tanks === undefined) {
...
​
                    // have the creep build, why do we check for a difference beetween the creep.room.name and the roomIndex is eluding me
...
​
                    // have the creep repair structures
					if (repair === undefined) {
...
​
                    // remote mining if sources in current room are depleted
